...
function renderTareas(){
  ...
  const cols = [
    { key:'espera', label:'⏳ En espera', items: all.filter(t=>t.status==='en_espera'||t.status==='pendiente'||!t.status).sort((a,b)=>{const p={alta:0,media:1,baja:2};return (p[a.priority]||1)-(p[b.priority]||1);}) },
    { key:'curso', label:'🔄 En curso',  items: all.filter(t=>t.status==='en_curso').sort((a,b)=>{const p={alta:0,media:1,baja:2};return (p[a.priority]||1)-(p[b.priority]||1);}) },
    { key:'done', label:'✅ Terminadas', items: all.filter(t=>t.status==='terminada').sort((a,b)=>(b.completedAt||'').localeCompare(a.completedAt||'')).slice(0,20) },
  ];
