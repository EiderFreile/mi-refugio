...
const IMPULSO_DONDE = ['Casa','Cama','Trabajo','Gym','Supermercado','Restaurante','Coche','Transporte público','Baño','Calle','Casa de alguien','Otro'];
const IMPULSO_QUE   = ['Mirando el móvil','Trabajando','Viendo una serie','Comiendo','Estudiando','Conduciendo','Haciendo compras','En el gym','Esperando','Hablando con alguien','Descansando','En el baño','De camino a algún sitio'];
...
function changeWeeklyCounter(id, delta) {
  if (!state.todayProgress.habitos) state.todayProgress.habitos = {};
  if (!state.todayProgress.habitos[id]) state.todayProgress.habitos[id] = { count: 0 };
  const current = state.todayProgress.habitos[id].count || 0;
  const newVal = Math.max(0, current + delta);
  DB.update(`refugio/registros/${todayKey()}/habitos/${id}`, { count: newVal, done: newVal > 0 })
    .then(() => saveDaySummary());
}
...
function getHabitProgress(h) {
  ...
  if (h.frecuencia === 'semanal') {
    ...
    keys.forEach(k => {
      const prog = k === today
        ? state.todayProgress?.habitos?.[h.id]
        : state.calendarData[k]?.habitos?.[h.id];
      if (!prog) return;
      if (prog.count) count += prog.count;
      else if (prog.done) count++;
    });
    return count;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderStaticHome();
  renderSettings();
  // Restaurar hora de reflexión en el input
  ...
  // Comprobar banner reflexión
  checkReflexionBanner();
  // Comprobar cambio de día cada minuto y al volver a la app
...
function initFirebaseListeners() {
  ...
  DB.listen('refugio/logros', data => {
    ...
    // Marcar todos los ya guardados en Firebase como vistos (no mostrar notificación)
    Object.keys(logrosDesbloqueados).forEach(id => {
      localStorage.setItem('logro_visto_' + id, '1');
    });
  });
}
...
function renderStaticHome() {
  ...
  if (greetEl) greetEl.innerHTML = `${greet}, <em>Eider</em>.`;
  ...
  if (msgEl) msgEl.textContent = DAILY_MESSAGES[dayOfYear % DAILY_MESSAGES.length];
  const natureEl = document.getElementById('hero-nature');
  if (natureEl) natureEl.textContent = getNatureIcon();
  renderMoodPicker();
  renderSummary();
  renderPasoDelDia();
}
      ...
      <div class="habit-item ${done ? 'completed' : ''}" id="habit-item-${h.id}">
        ...
        ${h.frecuencia === 'semanal' || h.frecuencia === 'mensual' ? `
          <div style="display:flex;gap:6px;align-items:center;">
            <button onclick="changeWeeklyCounter('${h.id}', -1)"
...
function saveDaySummary() {
  ...
  if (total > 0) {
    completed = dailyHabits.filter(h => {
      const p = prog[h.id] || {};
    ...
    // Sin hábitos diarios — color basado en estado de ánimo
    const moodColors = {
      feliz: 'excellent', bien: 'good', normal: 'normal',
      agobiada: 'difficult', cansada: 'normal', ansiosa: 'difficult',
      triste: 'difficult', enfadada: 'difficult'
    };
    if (state.todayMood) color = moodColors[state.todayMood] || 'normal';
    pct = 50;
  }
...
function renderHabitos() {
  ...
  list.innerHTML = state.habits.map(h => `
    <div class="habit-card" onclick="editHabitById('${h.id}')">
...
function renderConocete() {
  ...
  if (streak >= 2) {
    insights.push({ icon:'🔥', text:`Llevas <strong>${streak} días seguidos</strong> completando al menos un hábito. ¡Sigue así!` });
  }
  ...
  const wdAvgs = byWD.map((x,i) => ({ day:i, avg: x.count ? Math.round(x.sum/x.count) : -1 }));
  const bestWD = wdAvgs.filter(x => x.avg >= 0).sort((a,b) => b.avg - a.avg)[0];
  const worstWD = wdAvgs.filter(x => x.avg >= 0).sort((a,b) => a.avg - b.avg)[0];
  if (bestWD?.avg > 0) {
    insights.push({ icon:'⭐', text:`Los <strong>${dayNames[bestWD.day]}</strong> completas más hábitos (${bestWD.avg}% de media). Es tu mejor día.` });
  }
  if (worstWD && bestWD && worstWD.day !== bestWD.day && worstWD.avg >= 0) {
    insights.push({ icon:'🌧️', text:`Los <strong>${dayNames[worstWD.day]}</strong> sueles tener más dificultades (${worstWD.avg}% de media). Quizás necesitas más apoyo ese día.` });
  }

  // ── 3. Estado de ánimo más frecuente ────────────────────
  const moodCount = {};
  ...
  if (moodEntries.length > 0) {
    const [topId, topN] = moodEntries[0];
    const m = MOODS.find(x => x.id === topId);
    if (m) insights.push({ icon: m.emoji, text:`Tu estado más frecuente ha sido <strong>${m.label}</strong> (${topN} ${topN===1?'día':'días'}). Es cómo sueles sentirte.` });
  }

  // ── 4. Relación ánimo ↔ impulsos ────────────────────────
  const impulsosPorMood = {};
  dias.forEach(([,d]) => {
    if (!d.mood || !d.impulsos) return;
    if (!impulsosPorMood[d.mood]) impulsosPorMood[d.mood] = { total:0, mordidos:0, dias:0 };
    impulsosPorMood[d.mood].dias++;
    Object.values(d.impulsos).forEach(i => {
      impulsosPorMood[d.mood].total++;
      if (i.mordio) impulsosPorMood[d.mood].mordidos++;
    });
  });
  const moodConMasImpulsos = Object.entries(impulsosPorMood)
    .filter(([,v]) => v.dias >= 2)
    .sort((a,b) => (b[1].total/b[1].dias) - (a[1].total/a[1].dias))[0];
  if (moodConMasImpulsos) {
    const m = MOODS.find(x => x.id === moodConMasImpulsos[0]);
    if (m) insights.push({ icon:'🔍', text:`Cuando te sientes <strong>${m.label}</strong> aparecen más ganas de morderte la mejilla. Reconocer el patrón ya es un gran paso.` });
  }

  // ── 5. Impulsos totales ──────────────────────────────────
  const totalEvitados = getTotalEvitados(state);
  const totalImpulsos = Object.values(state.calendarData).reduce((acc,d) => {
    return acc + Object.values(d.impulsos||{}).length;
  }, 0);
  if (totalEvitados > 0) {
    const pctEvitados = totalImpulsos > 0 ? Math.round((totalEvitados/totalImpulsos)*100) : 0;
    insights.push({ icon:'💚', text:`Has evitado <strong>${totalEvitados} de ${totalImpulsos} impulsos</strong> en total (${pctEvitados}%). Cada uno que evitas es una victoria.` });
  }

  // ── 6. Calidad de comidas por día ───────────────────────
  const ratingScore = { muybien:5, bien:4, normal:3, regular:2, mal:1 };
  const comidaByWD = Array(7).fill(null).map(() => ({ sum:0, count:0 }));
  dias.forEach(([k,d]) => {
    if (!d.comidas) return;
    const wd = new Date(k + 'T12:00:00').getDay();
    Object.values(d.comidas).forEach(c => {
      if (c.rating && ratingScore[c.rating]) {
        comidaByWD[wd].sum += ratingScore[c.rating];
        comidaByWD[wd].count++;
      }
    });
  });
  const worstComidaWD = comidaByWD
    .map((x,i) => ({ day:i, avg: x.count >= 2 ? x.sum/x.count : null }))
    .filter(x => x.avg !== null)
    .sort((a,b) => a.avg - b.avg)[0];
  if (worstComidaWD) {
    insights.push({ icon:'🍽️', text:`Los <strong>${dayNames[worstComidaWD.day]}</strong> tiendes a comer peor. Puede que ese día necesites más mimo con la comida.` });
  }

  // ── 7. Comparativa semana actual vs anterior ─────────────
  const thisWeekKeys = weekKeys();
  const lastWeekKeys = Array.from({length:7}, (_,i) => {
    const d = new Date(thisWeekKeys[0]);
    d.setDate(d.getDate() - 7 + i);
    return d.toISOString().slice(0,10);
  });
  const avgPct = keys => {
    const valid = keys.map(k => state.calendarData[k]?.pct).filter(p => p !== undefined);
    return valid.length ? Math.round(valid.reduce((a,b)=>a+b,0)/valid.length) : null;
  };
  const thisAvg = avgPct(thisWeekKeys);
  const lastAvg = avgPct(lastWeekKeys);
  if (thisAvg !== null && lastAvg !== null) {
    const diff = thisAvg - lastAvg;
    if (diff > 5) insights.push({ icon:'📈', text:`Esta semana vas <strong>${diff} puntos mejor</strong> que la semana pasada (${thisAvg}% vs ${lastAvg}%). ¡Vas en buena dirección!` });
    else if (diff < -5) insights.push({ icon:'📉', text:`Esta semana vas un poco más floja que la anterior (${thisAvg}% vs ${lastAvg}%). Está bien, no siempre puede ser igual.` });
    else insights.push({ icon:'〰️', text:`Esta semana vas muy parecido a la anterior (${thisAvg}% vs ${lastAvg}%). Constancia es clave.` });
  }

  // ── 8. Hábito más constante ─────────────────────────────
  const habitCount = {};
  dias.forEach(([,d]) => {
    if (!d.habitos) return;
    Object.entries(d.habitos).forEach(([id, prog]) => {
      if (prog.done) habitCount[id] = (habitCount[id]||0)+1;
    });
  });
  const topHabit = Object.entries(habitCount).sort((a,b)=>b[1]-a[1])[0];
  if (topHabit) {
    const h = state.habits.find(x => x.id === topHabit[0]);
    if (h) insights.push({ icon: h.icon, text:`Tu hábito más constante es <strong>${h.name}</strong> — lo has completado ${topHabit[1]} ${topHabit[1]===1?'vez':'veces'}.` });
  }

  // ── 9. Días usando la app ───────────────────────────────
  const daysWithData = dias.filter(([,d]) => d.completed !== undefined || d.mood).length;
  insights.push({ icon:'📅', text:`Llevas <strong>${daysWithData} ${daysWithData===1?'día':'días'}</strong> usando Mi Refugio. Cada día que abres la app es un acto de cuidado hacia ti misma.` });

  // ── 10. Mejor día histórico ─────────────────────────────
  const best = dias.filter(([,d])=>d.pct!==undefined).sort((a,b)=>b[1].pct-a[1].pct)[0];
  if (best && best[1].pct > 0) {
    const bd = new Date(best[0]+'T12:00:00').toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'});
    insights.push({ icon:'🌟', text:`Tu mejor día fue el <strong>${bd}</strong> con un ${best[1].pct}% de hábitos completados.` });
  }
...
function checkLogros() {
  LOGROS_DEF.forEach(logro => {
    if (logrosDesbloqueados[logro.id]) return; // ya en Firebase
    if (logro.check(state)) {
      const yaVisto = localStorage.getItem('logro_visto_' + logro.id);
      logrosDesbloqueados[logro.id] = { unlockedAt: Date.now() };
      DB.set(`refugio/logros/${logro.id}`, { unlockedAt: Date.now() });
      if (!yaVisto) {
        localStorage.setItem('logro_visto_' + logro.id, '1');
...
// ═══════════════════════════════════════════════════════════
//  PASO DEL DÍA — hábito prioritario
// ═══════════════════════════════════════════════════════════

const NATURE_ICONS = ['🌿','🌸','🍃','🌱','🌾','🍀','🌼','🌺'];

function getNatureIcon() {
  const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(),0,0)) / 86400000);
  return NATURE_ICONS[dayOfYear % NATURE_ICONS.length];
}

const SELFCARE_TIPS = [
  { icon: '💧', texto: 'Bebe un vaso de agua ahora mismo', sub: 'La hidratación es cuidado' },
  { icon: '🌬️', texto: 'Respira hondo tres veces', sub: 'Inhala calma, exhala tensión' },
  { icon: '🚿', texto: 'Date una ducha calentita', sub: 'El agua caliente resetea el cuerpo' },
  { icon: '🛏️', texto: 'Cambia las sábanas hoy', sub: 'Dormir en ropa limpia es un regalo' },
  { icon: '🧴', texto: 'Aplícate crema en las manos', sub: 'Un pequeño gesto de cariño' },
  { icon: '🌿', texto: 'Abre una ventana y respira aire fresco', sub: 'Aunque sea un minuto' },
  { icon: '🧘', texto: 'Estira el cuello y los hombros', sub: 'Suelta la tensión acumulada' },
  { icon: '☕', texto: 'Prepárate algo calentito sin prisas', sub: 'Un momento solo para ti' },
  { icon: '🌞', texto: 'Sal a tomar un poco de sol', sub: 'Aunque sea cinco minutos' },
  { icon: '📵', texto: 'Pon el móvil boca abajo 10 minutos', sub: 'El mundo puede esperar' },
  { icon: '🧹', texto: 'Ordena un rincón pequeño', sub: 'El orden fuera calma el orden dentro' },
  { icon: '🎵', texto: 'Pon una canción que te guste mucho', sub: 'La música es medicina' },
  { icon: '🌙', texto: 'Prepara tu ropa para mañana esta noche', sub: 'Regálate calma por la mañana' },
  { icon: '✍️', texto: 'Escribe tres cosas que te gustan de ti', sub: 'Aunque cueste encontrarlas' },
  { icon: '🦷', texto: 'Usa el hilo dental hoy', sub: 'Cuidarte también es esto' },
  { icon: '🤸', texto: 'Date un pequeño paseo de 10 minutos', sub: 'Mover el cuerpo mueve la mente' },
  { icon: '🧖', texto: 'Lávate la cara con calma', sub: 'Sin prisas, con mimo' },
  { icon: '📖', texto: 'Lee aunque sean dos páginas', sub: 'Darle a tu mente otro mundo' },
  { icon: '💤', texto: 'Acuéstate 30 minutos antes esta noche', sub: 'El descanso es productividad' },
  { icon: '🌸', texto: 'Manda un mensaje cariñoso a alguien', sub: 'Dar también es recibir' },
  { icon: '🥗', texto: 'Come algo que sepa bien y te nutra', sub: 'Tu cuerpo merece lo mejor' },
  { icon: '🕯️', texto: 'Enciende una vela o un incienso', sub: 'Crea un ambiente que te cuide' },
  { icon: '🧃', texto: 'Come una fruta que te apetezca', sub: 'Pequeños nutrientes, gran energía' },
  { icon: '💆', texto: 'Date un masaje en las sienes', sub: 'Tres círculos suaves, ya' },
  { icon: '🌊', texto: 'Escucha sonidos de naturaleza unos minutos', sub: 'Lluvia, olas, bosque...' },
  { icon: '👟', texto: 'Ponte ropa cómoda si puedes', sub: 'El cuerpo agradece la libertad' },
  { icon: '🪴', texto: 'Riega tus plantas si las tienes', sub: 'Cuidar vida es cuidarse' },
  { icon: '😴', texto: 'Échate una siesta corta si lo necesitas', sub: '20 minutos son suficientes' },
  { icon: '🤲', texto: 'Lávate las manos despacio y con conciencia', sub: 'Un ritual pequeño y bonito' },
  { icon: '🌤️', texto: 'Mira el cielo un momento', sub: 'Siempre hay algo bonito ahí arriba' },
];

function renderPasoDelDia() {
  const content = document.getElementById('paso-content');
  const card = document.getElementById('paso-card');
  if (!content) return;

  // Icono naturaleza en hero
  const natureEl = document.getElementById('hero-nature');
  if (natureEl) natureEl.textContent = getNatureIcon();

  // Elegir tip del día (cambia cada día)
  const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(),0,0)) / 86400000);
  const tip = SELFCARE_TIPS[dayOfYear % SELFCARE_TIPS.length];

  card.classList.remove('completado');

  content.innerHTML = `
    <div class="paso-habito">
      <div class="paso-icon" style="background:var(--c-sage-dim);font-size:28px;">
        ${tip.icon}
      </div>
      <div>
        <div class="paso-nombre" style="font-size:18px;">${tip.texto}</div>
        <div class="paso-sub">${tip.sub}</div>
      </div>
    </div>
    <button class="btn-hecho" id="btn-hecho-selfcare" onclick="completarSelfcare()">
      Lo he hecho ✓
    </button>`;

  // Si ya se marcó hoy
  if (localStorage.getItem('selfcare_' + todayKey())) {
    card.classList.add('completado');
    content.innerHTML = `
      <div class="paso-habito">
        <div class="paso-icon" style="background:var(--c-sage-dim);font-size:28px;">${tip.icon}</div>
        <div>
          <div class="paso-nombre" style="font-size:18px;">${tip.texto}</div>
          <div class="paso-sub" style="color:var(--c-sage);">¡Hecho hoy! 💚</div>
        </div>
      </div>`;
  }
}

function completarSelfcare() {
  localStorage.setItem('selfcare_' + todayKey(), '1');
  lanzarHoja();
  const btn = document.getElementById('btn-hecho-selfcare');
  if (btn) { btn.textContent = '¡Genial! 🌿'; btn.disabled = true; }
  setTimeout(() => renderPasoDelDia(), 600);
}
...
function lanzarHoja() {
  ...
  for (let i = 0; i < 3; i++) {
    setTimeout(() => {
      ...
      leaf.textContent = hojas[Math.floor(Math.random() * hojas.length)];
...
function toggleTodosHabitos() {
  const wrap = document.getElementById('todos-habitos-wrap');
  const btn  = document.getElementById('toggle-habitos-btn');
  if (!wrap || !btn) return;
  const visible = wrap.style.display !== 'none';
  wrap.style.display = visible ? 'none' : 'block';
  btn.textContent = visible ? 'Ver todos ↓' : 'Ocultar ↑';
}
