...
function renderHoy(){
  ...
  area.innerHTML=`
    ...
    <div class="hoy-section">
      ...
      ${hoyPendientes.map((t,i)=>renderTareaCard(t,{idx:i,colLen:hoyPendientes.length,section:'curso'})).join('')}
    </div>`:''}

    ${overdue.length?`
    <div class="hoy-section">
      <div class="hoy-section-title">⏰ Vencidas</div>
      ${overdue.map((t,i)=>renderTareaCard(t,{idx:i,colLen:overdue.length,section:'vencidas'})).join('')}
    </div>`:''}
...
function renderTareaCard(t, opts={}) {
  ...
  const orderBtns = opts.idx !== undefined ? `<div style="display:flex;flex-direction:column;gap:2px;flex-shrink:0;" onclick="event.stopPropagation()">
    ${opts.idx > 0 ? `<button style="background:none;border:none;cursor:pointer;font-size:11px;color:var(--text-muted);padding:0 2px;line-height:1;" onclick="moveTarea('${t.id}','up')">↑</button>` : '<span style="width:16px;"></span>'}
    ${opts.idx < (opts.colLen||0)-1 ? `<button style="background:none;border:none;cursor:pointer;font-size:11px;color:var(--text-muted);padding:0 2px;line-height:1;" onclick="moveTarea('${t.id}','down')">↓</button>` : '<span style="width:16px;"></span>'}
  </div>` : '';
