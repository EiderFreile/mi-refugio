...
function renderChecklistsHome() {
  ...
  // Guardar qué listas están abiertas antes de re-renderizar
  const openIds = new Set();
  document.querySelectorAll('.checklist-items.open').forEach(el => {
    openIds.add(el.id.replace('items-', ''));
  });
  ...
  container.innerHTML = today.map(cl => {
    ...
    return `
      <div class="checklist-card" id="clcard-${cl.id}">
        ...
        <div class="checklist-items ${openIds.has(cl.id) ? 'open' : ''}" id="items-${cl.id}">
          ${items.map(it => {
...
function renderNotesHome() {
  const container = document.getElementById('notes-home');
  if (!container) return;
  ...
  container.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin:20px 0 10px;">
      ...
      <button onclick="navigateTo('notas')"
        style="font-size:12px;color:var(--lavender);background:none;border:none;
    ...
    <div style="display:flex;flex-direction:column;gap:8px;">
      ${notes.map(n => {
        ...
        return `
          <div onclick="openNoteSheet(${JSON.stringify(n).replace(/"/g,'&quot;')})"
            style="background:${bg};border-radius:12px;padding:12px 14px;cursor:pointer;
                   transition:transform 0.15s;-webkit-tap-highlight-color:transparent;"
            onactive="this.style.transform='scale(0.98)'">
            <div style="font-size:13px;color:rgba(26,26,26,0.85);line-height:1.5;">${preview}</div>
          </div>`;
      }).join('')}
    </div>`;
}
...
function renderNotes() {
  const area = document.querySelector('#screen-notas .scroll-area');
  if (!area) return;
  ...
  area.innerHTML = `<div class="notes-grid" id="notes-grid">
    ${notes.map(n => {
      const bg = COLORS.find(c => c.id === n.color)?.hex || COLORS[1].hex;
