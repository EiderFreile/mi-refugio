...
function renderHoy(){
  ...
  // Tareas HOY pendientes
  const hoyTareas = allTareas.filter(t => {
    if(!t.hoy) return false;
    if(t.status==='terminada') return t.completedAt===today;
    return true;
  });
  const hoyTerminadas = hoyTareas.filter(t=>t.status==='terminada');
  const hoyPendientes = hoyTareas.filter(t=>t.status!=='terminada');
  const pct = hoyTareas.length ? Math.round((hoyTerminadas.length/hoyTareas.length)*100) : 0;

  // Vencidas
  const overdue = allTareas.filter(t=>t.status!=='terminada'&&t.date&&t.date<today&&!t.hoy);

  // En espera: todas las tareas con status en_espera (con o sin pasos)
  const enEsperaItems = [];
  allTareas.forEach(t=>{
    if(t.status==='terminada'||t.status==='en_curso') return;
...
function completarPasoYSiguiente(tareaId, pasoId){
  const t = state.tareas[tareaId]; if(!t) return;
  DB.update(`refugio2/tareas/${tareaId}/pasos/${pasoId}`, {done: true, estado: 'done'}).then(()=>{
    // Check remaining pasos
    const pasos = t.pasos ? Object.values(t.pasos) : [];
    const restantes = pasos.filter(p=>p.id!==pasoId&&!p.done&&p.estado!=='done');
    if(restantes.length===0){
    ...
    <div style="background:var(--bg);border-radius:8px;padding:10px;margin-bottom:8px;border:1px solid var(--border);">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <select class="input-field" style="flex:1;" onchange="editingPasos[${i}].estado=this.value">
          <option value="en_espera" ${(!p.estado||p.estado==='en_espera'||p.estado==='pendiente')&&!p.done?'selected':''}>⏳ En espera</option>
          <option value="done" ${p.done?'selected':''}>✓ Hecho</option>
        </select>
      </div>
    </div>`).join('');
}
...
function openTareaSheet(t){
  ...
  document.getElementById('tarea-status').value=t?.status||'en_espera';
...
function saveTarea(){
  ...
  const tipo=document.getElementById('tarea-tipo').value;
  const pasos = tipo==='pasos' && editingPasos.length
    ? Object.fromEntries(editingPasos.filter(p=>p.text.trim()).map(p=>[p.id,{
        id:p.id, text:p.text.trim(), date:p.date||'',
        done: p.estado==='done' || p.done || false,
        estado: p.estado||'pendiente'
      }]))
    : null;
  const statusVal = document.getElementById('tarea-status').value;
  const isEnCurso = statusVal === 'en_curso';
  DB.set(`refugio2/tareas/${id}`,{
    ...
    hoy: isEnCurso,
    pasos,
    completedAt:status==='terminada'?(state.tareas[id]?.completedAt||todayKey()):null,
