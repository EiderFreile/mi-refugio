...
function renderPasoDelDia() {
  ...
  if (!state.habits.length) {
    content.innerHTML = `
      <div style="text-align:center;padding:16px 0;color:var(--c-text-muted);font-size:14px;">
        Crea tu primer hábito para empezar 🌱
      </div>`;
    return;
  }

  // Encontrar el primer hábito diario no completado
  const dailyHabits = state.habits.filter(h => !h.frecuencia || h.frecuencia === 'diario');
  const pendiente = dailyHabits.find(h => {
    const prog = state.todayProgress?.habitos?.[h.id] || {};
    if (esHabitoEspecial(h)) return false;
    if (h.type === 'boolean') return !prog.done;
    if (h.type === 'counter') return (prog.current || 0) < (h.goal || 1);
    return false;
  });
  ...
  if (!pendiente) {
    ...
    content.innerHTML = `
      <div style="text-align:center;padding:8px 0;">
        ...
        <div style="font-family:'Playfair Display',serif;font-size:20px;font-weight:500;color:var(--c-sage);">
          ¡Lo has dado todo hoy!
        </div>
        <div style="font-size:13px;color:var(--c-text-muted);margin-top:8px;">
  ...
  const prog = state.todayProgress?.habitos?.[pendiente.id] || {};
  const current = pendiente.type === 'counter' ? (prog.current || 0) : 0;
  const remaining = pendiente.type === 'counter' ? pendiente.goal - current : 1;

  content.innerHTML = `
    <div class="paso-habito">
      <div class="paso-icon" style="background:${pendiente.color}25;">
        ${pendiente.icon}
      </div>
      <div>
        <div class="paso-nombre">${pendiente.name}</div>
        ${pendiente.type === 'counter' ? `
          <div class="paso-sub">${current} de ${pendiente.goal} ${pendiente.unit || ''} · Faltan ${remaining}</div>
        ` : `
          <div class="paso-sub">Pendiente para hoy</div>
        `}
      </div>
    </div>
    ${pendiente.type === 'counter' ? `
      <div style="margin-bottom:16px;">
        <div style="height:6px;background:var(--c-beige);border-radius:99px;overflow:hidden;">
          <div style="height:100%;width:${Math.round((current/pendiente.goal)*100)}%;
                      background:${pendiente.color};border-radius:99px;transition:width 0.5s ease;"></div>
        </div>
        <div style="display:flex;gap:8px;margin-top:12px;">
          <button onclick="pasoChangeCounter('${pendiente.id}', -1)"
            style="flex:1;padding:14px;border-radius:12px;border:1.5px solid var(--c-border);
                   background:none;font-size:20px;cursor:pointer;">−</button>
          <button onclick="pasoChangeCounter('${pendiente.id}', 1)"
            style="flex:2;padding:14px;border-radius:12px;border:none;
                   background:var(--c-text);color:var(--c-bg);font-size:15px;
    ...
    ` : `
      <button class="btn-hecho" id="btn-hecho-${pendiente.id}"
              onclick="completarPaso('${pendiente.id}')">
        Hecho ✓
      </button>
    `}`;
}
...
function completarPaso(id) {
  const btn = document.getElementById('btn-hecho-' + id);
  if (btn) {
  ...
  setTimeout(() => {
    toggleHabit(id, false);
    setTimeout(() => renderPasoDelDia(), 600);
  }, 400);
}

function pasoChangeCounter(id, delta) {
  changeCounter(id, delta);
  setTimeout(() => renderPasoDelDia(), 300);
}
