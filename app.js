...
function toggleTareaHoy(id, currentHoy) {
  DB.update(`refugio2/tareas/${id}`, {hoy: !currentHoy});
  showToast(!currentHoy ? 'Añadida a Hoy ☀️' : 'Quitada de Hoy');
}
...
function renderHoy(){
  ...
  // En espera: tareas en espera + pasos en espera de cualquier tarea
  const enEsperaItems = [];
  allTareas.forEach(t=>{
    if(t.status==='terminada') return;
    if(t.status==='en_espera' && !t.hoy){
      enEsperaItems.push({tipo:'tarea', tarea:t, texto:t.name, nota:t.notes||''});
    }
    // Pasos en espera o pendientes de tareas no terminadas
    if(t.pasos) Object.values(t.pasos).forEach(p=>{
      if(!p.done && p.estado==='en_espera'){
        enEsperaItems.push({tipo:'paso', tarea:t, paso:p, texto:`${t.name}`, nota:p.text});
      }
    });
  });
  ...
  area.innerHTML=`
    ...
    <div class="hoy-section">
      ...
      ${enEsperaItems.map(item=>`
        <div class="hoy-task waiting" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;gap:10px;">
            <span style="font-size:16px;flex-shrink:0;">⏳</span>
            <div style="flex:1;min-width:0;cursor:pointer;" onclick="openTareaSheet(${JSON.stringify(item.tarea).replace(/"/g,'&quot;')})">
              <div style="font-size:14px;font-weight:600;">${item.tarea.name}</div>
              ${item.tipo==='paso'?`<div style="font-size:12px;color:var(--text-muted);margin-top:2px;">⏳ ${item.nota}</div>`:''}
              ${item.tipo==='tarea'&&item.tarea.notes?`<div style="font-size:12px;color:var(--text-muted);margin-top:2px;">${item.tarea.notes.slice(0,50)}</div>`:''}
            </div>
            ${item.tipo==='tarea'?`<button onclick="event.stopPropagation();toggleTareaHoy('${item.tarea.id}',false)"
              style="background:var(--bg);border:1px solid var(--border);border-radius:99px;padding:3px 8px;font-size:12px;cursor:pointer;flex-shrink:0;">☀️</button>`:''}
          </div>
          ${item.tipo==='paso'?`
          <div style="margin-top:8px;padding-left:26px;">
            <div style="display:flex;align-items:center;gap:8px;cursor:pointer;" onclick="togglePasoCheck('${item.tarea.id}','${item.paso.id}',false)">
              ...
              <span style="font-size:13px;color:var(--text);">Marcar como hecho</span>
              ${item.paso.date?`<span style="font-size:11px;color:var(--text-muted);margin-left:auto;">📅 ${item.paso.date}</span>`:''}
            </div>
          </div>`:''}
        </div>`).join('')}
    </div>`:''}
...
function saveTarea(){
  ...
  DB.set(`refugio2/tareas/${id}`,{
    ...
    hoy: isEnCurso || document.getElementById('tarea-hoy').checked,
    pasos,
