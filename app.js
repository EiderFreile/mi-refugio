...
function moveTarea(id, dir) {
  ...
  if (idx === -1) { showToast(`⚠️ No la encuentro (estado real:"${t.status}" → busco:"${estado}", col tiene ${col.length})`, 6000); return; }
  const swapIdx = dir==='up' ? idx-1 : idx+1;
  if (swapIdx<0 || swapIdx>=col.length) { showToast(`En el extremo — idx:${idx}/${col.length-1} col:[${col.map(x=>x.name).join(', ')}]`, 8000); return; }
  ...
  other.order = aOrder;
  showToast(`▲▼ idx${idx}→${swapIdx} | "${t.name}"(${aOrder}→${bOrder}) ↔ "${other.name}"(${bOrder}→${aOrder}) | col:${col.map(x=>x.name).join(',')}`, 8000);
  console.log('moveTarea debug', {id, dir, estado, col: col.map(x=>({name:x.name,status:x.status,order:x.order})), idx, swapIdx});
  try {
    renderTareas();
    renderHoy();
  } catch(e) {
