...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  function adjustLayout() {
    ...
    if (!nav || !modeBar) return;
    if (window.innerWidth >= 768) {
      if (!modeBar.contains(nav)) modeBar.appendChild(nav);
    } else {
      const app = document.getElementById('app');
      if (!app.contains(nav) || modeBar.contains(nav)) app.appendChild(nav);
    }
  }
  adjustLayout();
...
function getEstadoEfectivo(t) {
  ...
  const pasos = Object.values(t.pasos);
  // Siguiente paso no terminado
  const nextPaso = pasos.find(p => p.estado !== 'terminada' && !p.done);
  if (!nextPaso) return 'terminada'; // todos hechos
  return nextPaso.estado || 'en_espera';
}
...
function togglePasoCheck(tareaId, pasoId, currentDone){
  ...
  if(restantes.length === 0 && newDone){
    DB.update(`refugio2/tareas/${tareaId}`,{status:'terminada', completedAt:todayKey(), hoy:false});
    ...
    const newHoy = nextEstado === 'en_curso';
    DB.update(`refugio2/tareas/${tareaId}`,{
      status: nextEstado,
      hoy: newHoy,
      completedAt: null
...
function toggleTareaHoy(id, currentHoy) {
  ...
  if (!t) return;
  const newHoy = !currentHoy;
  const newStatus = newHoy ? 'en_curso' : (t.status === 'en_curso' ? 'en_espera' : t.status);
  DB.update(`refugio2/tareas/${id}`, {hoy: newHoy, status: newStatus});
  showToast(newHoy ? 'En curso ☀️' : 'Movida a En espera');
}
    ...
    <div class="hoy-section">
      ...
      ${overdue.map(t=>`
        <div class="hoy-task overdue" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
          ...
          <button onclick="event.stopPropagation();toggleTareaHoy('${t.id}',false)"
            style="background:var(--bg);border:1px solid var(--border);border-radius:99px;padding:3px 8px;font-size:12px;cursor:pointer;margin-left:4px;">☀️</button>
        </div>`).join('')}
    </div>`:''}
      ...
      ${enEsperaItems.map(item=>{
        ...
        return `<div class="hoy-task waiting" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;gap:10px;">
            ...
            <button onclick="event.stopPropagation();toggleTareaHoy('${t.id}',${!!t.hoy})"
              style="background:${t.hoy?'var(--lav)':'var(--bg)'};border:1px solid ${t.hoy?'var(--lav)':'var(--border)'};border-radius:99px;padding:3px 8px;font-size:12px;cursor:pointer;flex-shrink:0;color:${t.hoy?'white':'inherit'};">☀️</button>
          </div>
          ${nextPaso?`
          <div style="margin-top:8px;padding-left:26px;">
