...
function toggleTareaCompleto(id) {
  const t = state.tareas[id]; if(!t) return;
      ...
      ${hoyPendientes.map(t=>{
        ...
        return `<div class="hoy-task" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;gap:10px;">
            ...
            <span style="flex:1;font-size:14px;font-weight:600;cursor:pointer;" onclick="openTareaSheet(state.tareas['${t.id}'])">${t.name}</span>
      ...
      ${enEsperaItems.map(item=>{
        ...
        return `<div class="hoy-task waiting" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div style="flex:1;min-width:0;cursor:pointer;" onclick="openTareaSheet(state.tareas['${t.id}'])">
  ...
  return `<div id="kcard-${t.id}" class="hoy-task${isOverdue?' overdue':''}" style="flex-direction:column;align-items:stretch;margin-bottom:8px;${t.status==='terminada'?'opacity:0.6;':''}"
    ...
    <div style="display:flex;align-items:center;gap:10px;">
      <div class="check-box ${t.status==='terminada'?'checked':''}" style="flex-shrink:0;"
        onclick="event.stopPropagation();toggleTareaCompleto('${t.id}')">
        ${checkSvg}
      </div>
      <span style="flex:1;font-size:14px;font-weight:600;cursor:pointer;${t.status==='terminada'?'text-decoration:line-through;color:var(--text-muted)':''}"
        onclick="openTareaSheet(state.tareas['${t.id}'])">${priIcon[t.priority]||''} ${t.name}</span>
      ${statusBtns}
      ${orderBtns}
    </div>
