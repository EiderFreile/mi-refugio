...
const DAY_TABS=[{id:'inicio',ico:'🏠',lbl:'Inicio'},{id:'listas',ico:'✅',lbl:'Listas'},{id:'porhacer',ico:'✍️',lbl:'Por hacer'},{id:'notas',ico:'📝',lbl:'Notas'},{id:'gastos',ico:'💰',lbl:'Gastos'}];
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('fab').addEventListener('click', ()=>{
    if(state.tab==='porhacer') openPorHacerSheet();
    else openQuickAdd();
  });
  initListeners();
  renderGreeting();
...
function initListeners(){
  ...
  DB.listen('refugio2/porhacer',d=>{state.porhacer=d||{};renderPorHacer();});
...
function addTareaCat(){
  ...
  const emoji = document.getElementById('tarea-cat-emoji').value.trim() || '📁';
  ...
  DB.set(`refugio2/tareaCats/${id}`,{id,name,emoji}).then(()=>{
    ...
    document.getElementById('tarea-cat-emoji').value='';
