...
function openQuickAddWork(){
  ...
  if(state.tab==='agenda'){openEventoSheet(null);return;}
  if(state.tab==='recordar'){openRecordarSheet(null);return;}
  showQuickSheet([
    {ico:'✅',lbl:'Tarea',fn:'openTareaSheet(null)'},
    {ico:'📅',lbl:'Evento',fn:'openEventoSheet(null)'},
    {ico:'🧠',lbl:'Recordar',fn:'openRecordarSheet(null)'},
  ]);
}
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
        ...
        <div style="display:flex;gap:6px;align-items:center;flex-shrink:0;">
          ${t.status!=='terminada'?`<button onclick="event.stopPropagation();toggleTareaHoy('${t.id}',${!!t.hoy})"
            style="background:${t.hoy?'var(--lav)':'var(--bg)'};border:1px solid ${t.hoy?'var(--lav)':'var(--border)'};border-radius:99px;padding:3px 8px;font-size:12px;cursor:pointer;-webkit-tap-highlight-color:transparent;" title="Añadir a Hoy">
            ☀️
          </button>`:''}
...
function saveTarea(){
  ...
  DB.set(`refugio2/tareas/${id}`,{id,name,priority:document.getElementById('tarea-priority').value,status,date:document.getElementById('tarea-date').value,cat:document.getElementById('tarea-cat').value,notes:document.getElementById('tarea-notes').value.trim(),hoy:document.getElementById('tarea-hoy').checked,completedAt: status==='terminada' ? (state.tareas[id]?.completedAt || todayKey()) : null, createdAt:state.tareas[id]?.createdAt||Date.now()}).then(()=>{closeTareaSheet();showToast('Tarea guardada ✓');});
}
