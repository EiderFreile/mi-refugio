...
const WORK_TABS=[{id:'hoy',ico:'☀️',lbl:'Hoy'},{id:'tareas',ico:'✅',lbl:'Tareas'},{id:'agenda',ico:'📅',lbl:'Agenda'}];
...
function renderMiniCal(){
  ...
  const eventDays = new Set(Object.values(state.eventos).map(e=>e.date));
  ...
  for(let d=1;d<=days;d++){
    ...
    grid+=`<div class="mini-cal-day ${isToday?'today':''} ${hasEvents?'has-events':''}"
      onclick="selectDay('${key}');navigateTo('agenda')">${d}</div>`;
  }
  ...
  container.innerHTML=`
    <div class="mini-cal">
      ...
      ${Object.values(state.eventos).filter(e=>e.date===today).length ? `
        <div style="margin-top:12px;border-top:1px solid var(--border);padding-top:10px;">
          <div style="font-size:11px;font-weight:700;color:var(--text-muted);margin-bottom:6px;">HOY</div>
          ${Object.values(state.eventos).filter(e=>e.date===today).sort((a,b)=>(a.time||'').localeCompare(b.time||'')).map(e=>`
            <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;cursor:pointer;" onclick="openEventoSheet(${JSON.stringify(e).replace(/"/g,'&quot;')})">
              <span style="font-size:11px;font-weight:700;color:var(--lav);width:36px;flex-shrink:0;">${e.time||'·'}</span>
              <span style="font-size:12px;font-weight:500;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${TIPO_ICONS[e.tipo]||'📅'} ${e.name}</span>
            </div>`).join('')}
        </div>` : ''}
    </div>`;
}
...
function renderHoy(){
  ...
  const hoyTareas = allTareas.filter(t => {
    ...
    if(t.status === 'terminada') return t.completedAt === today; // completada hoy → mostrar
    return true; // pendiente → siempre mostrar
  });
  ...
  area.innerHTML=`
    ...
    ${!todayEvents.length&&!hoyPendientes.length&&!overdue.length&&!enEspera.length?`<div class="empty-state"><div class="empty-icon">✨</div><p>Todo al día.<br>Sin tareas urgentes hoy.</p></div>`:''}
    <div class="hoy-section" style="margin-top:16px;">
      ...
      ${hoyTerminadas.length?`<div style="margin-top:10px;">${hoyTerminadas.map(t=>`
        <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);opacity:0.6;">
          <div class="check-box checked"><svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M1.5 5.5L4 8L9.5 2.5" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></div>
  ...
  area.innerHTML=all.map(t=>{
    ...
    return`<div class="task-card ${isOverdue?'overdue':priColors[t.priority]}" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
        ...
        <div style="display:flex;gap:6px;align-items:center;flex-shrink:0;">
          ${t.status!=='terminada'?`<button onclick="event.stopPropagation();toggleTareaHoy('${t.id}',${!!t.hoy})"
            style="background:${t.hoy?'var(--lav)':'var(--bg)'};border:1px solid ${t.hoy?'var(--lav)':'var(--border)'};border-radius:99px;padding:3px 8px;font-size:12px;cursor:pointer;-webkit-tap-highlight-color:transparent;">☀️</button>`:''}
      ...
      <div class="task-meta">
        ...
        ${t.cat?`<span class="tag tag-gray">${getCatName(t.cat)}</span>`:''}
        ${t.hoy?`<span class="tag tag-lav">HOY</span>`:''}
        ${pasos.length?`<span class="tag tag-gray">📋 ${pasosDone}/${pasos.length} pasos</span>`:''}
      </div>
      ${pasos.length?`<div style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px;">
        ${pasos.map(p=>`<div style="display:flex;align-items:center;gap:8px;padding:4px 0;" onclick="event.stopPropagation();togglePasoCheck('${t.id}','${p.id}',${!!p.done})">
          <div class="check-box ${p.done?'checked':''}" style="width:16px;height:16px;border-radius:4px;flex-shrink:0;">
          ...
          <span style="font-size:13px;${p.done?'text-decoration:line-through;color:var(--text-muted)':'color:var(--text)'};">${p.text}</span>
          ${p.date?`<span style="font-size:11px;color:var(--text-muted);margin-left:auto;">📅 ${p.date}</span>`:''}
        </div>`).join('')}
      </div>`:''}
      ${t.notes&&!pasos.length?`<div style="font-size:12px;color:var(--text-muted);margin-top:6px;">${t.notes.slice(0,80)}${t.notes.length>80?'...':''}</div>`:''}
    </div>`;
  }).join('');
}
  ...
  document.getElementById('pasos-list').innerHTML = editingPasos.map((p,i)=>`
    <div style="background:var(--bg);border-radius:8px;padding:10px;margin-bottom:8px;border:1px solid var(--border);">
      ...
      <input class="input-field" type="date" value="${p.date||''}" 
        style="width:100%;" oninput="editingPasos[${i}].date=this.value"
...
function saveTarea(){
  ...
  const pasos = tipo==='pasos' && editingPasos.length
    ? Object.fromEntries(editingPasos.filter(p=>p.text.trim()).map(p=>[p.id,{id:p.id,text:p.text.trim(),date:p.date||'',done:p.done||false}]))
    : null;
