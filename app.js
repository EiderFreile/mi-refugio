...
function completarPasoYSiguiente(tareaId, pasoId){
  ...
  DB.update(`refugio2/tareas/${tareaId}/pasos/${pasoId}`, {done: true, estado: 'terminada'}).then(()=>{
    setTimeout(()=>{
      ...
      const restantes = pasos.filter(p=>!p.done && p.estado!=='terminada' && p.id!==pasoId);
      if(restantes.length===0){
        DB.update(`refugio2/tareas/${tareaId}`,{status:'terminada',completedAt:todayKey(),hoy:false});
        showToast('¡Tarea completada! 🎉',3000);
      } else {
        syncTareaStatus(tareaId);
        showToast(`✓ Hecho · Siguiente: ${restantes[0].text}`);
      }
    }, 300);
  });
}
