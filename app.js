...
function renderTareaCard(t, opts={}) {
  ...
  const orderBtns = opts.idx !== undefined ? `<div class="order-btns" onclick="event.stopPropagation()">
    ${opts.idx > 0 ? `<button class="order-btn" onclick="moveTarea('${t.id}','up')">↑</button>` : ''}
    ${opts.idx < (opts.colLen||0)-1 ? `<button class="order-btn" onclick="moveTarea('${t.id}','down')">↓</button>` : ''}
  </div>` : '';

  return `<div id="kcard-${t.id}" class="hoy-task${isOverdue?' overdue':''}" style="flex-direction:column;align-items:stretch;margin-bottom:8px;${t.status==='terminada'?'opacity:0.6;':''}"
    draggable="true"
    ...
    <div style="display:flex;align-items:center;gap:10px;">
      <div class="check-box ${t.status==='terminada'?'checked':''}" style="flex-shrink:0;"
        onclick="event.stopPropagation();setTareaStatus('${t.id}',${t.status==='terminada'?`'en_espera'`:`'terminada'`})">
        ${checkSvg}
      </div>
      <span style="flex:1;font-size:14px;font-weight:600;cursor:pointer;${t.status==='terminada'?'text-decoration:line-through;color:var(--text-muted)':''}"
        onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">${priIcon[t.priority]||''} ${t.name}</span>
      ${statusBtns}
      ${orderBtns}
    </div>

    ${(t.date || t.cat) ? `<div style="display:flex;gap:8px;margin-top:4px;padding-left:30px;flex-wrap:wrap;">
    ...
    ${t.notes && !pasos.length ? `<div style="font-size:12px;color:var(--text-muted);margin-top:4px;padding-left:30px;">${t.notes.slice(0,80)}${t.notes.length>80?'...':''}</div>` : ''}
...
function renderTareas(){
  ...
  const cols = [
    { label:'⏳ En espera', items: all.filter(t=>t.status==='en_espera'||t.status==='pendiente'||!t.status).sort((a,b)=>{const p={alta:0,media:1,baja:2};return (p[a.priority]||1)-(p[b.priority]||1);}) },
    { label:'🔄 En curso',  items: all.filter(t=>t.status==='en_curso').sort((a,b)=>{const p={alta:0,media:1,baja:2};return (p[a.priority]||1)-(p[b.priority]||1);}) },
    { label:'✅ Terminadas', items: all.filter(t=>t.status==='terminada').sort((a,b)=>(b.completedAt||'').localeCompare(a.completedAt||'')).slice(0,20) },
  ];
  ...
  board.innerHTML = `<div class="kanban-board">${cols.map(col=>`
    <div class="kanban-col">
      ...
      ${col.items.length
        ? getSortedTareas(col.items).map((t,i,arr)=>renderTareaCard(t,{idx:i,colLen:arr.length})).join('')
