...
function renderTareaCard(t, opts={}) {
  ...
  return `<div id="kcard-${t.id}" class="hoy-task${isOverdue?' overdue':''}" style="flex-direction:column;align-items:stretch;margin-bottom:8px;${t.status==='terminada'?'opacity:0.6;':''}cursor:pointer;"
    draggable="true"
    ondragstart="onDragStart(event,'${t.id}')"
    ondragover="onDragOver(event,'${t.id}')"
    ondragend="onDragEnd(event)"
    ondrop="onDrop(event,'${t.id}')"
    onclick="openTareaSheet(state.tareas['${t.id}'])">
    ...
    <div style="display:flex;align-items:center;gap:10px;">
      ...
      <span style="flex:1;font-size:14px;font-weight:600;${t.status==='terminada'?'text-decoration:line-through;color:var(--text-muted)':''}">${priIcon[t.priority]||''} ${t.name}</span>
      ${statusBtns}
      ${orderBtns}
    </div>
  ...
  board.innerHTML = `<div class="kanban-board">${cols.map(col=>`
    <div class="kanban-col">
      <div class="kanban-col-title">
        <span>${col.label}</span>
        <span style="font-size:11px;font-weight:700;background:var(--border);color:var(--text-muted);padding:3px 8px;border-radius:99px;margin-left:auto;">${col.items.length}</span>
      </div>
      ${col.items.length
