...
function completarPasoYSiguiente(tareaId, pasoId){
  const t = state.tareas[tareaId]; if(!t) return;
  // Construir pasos actualizados localmente sin esperar Firebase
  const pasosActuales = t.pasos ? Object.values(t.pasos) : [];
  const pasosActualizados = pasosActuales.map(p =>
    p.id === pasoId ? {...p, done:true, estado:'terminada'} : p
  );
  const restantes = pasosActualizados.filter(p => !p.done && p.estado !== 'terminada');

  DB.update(`refugio2/tareas/${tareaId}/pasos/${pasoId}`, {done:true, estado:'terminada'});
