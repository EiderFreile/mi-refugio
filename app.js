...
const WORK_TABS=[{id:'hoy',ico:'☀️',lbl:'Resumen'},{id:'tareas',ico:'✅',lbl:'Tareas'},{id:'agenda',ico:'📅',lbl:'Agenda'},{id:'wetlease',ico:'✈️',lbl:'Wet Lease'}];
...
let state={
  ...
  tareas:{}, eventos:{}, tareaCats:{}, porhacer:{}, wlIn:{}, wlOut:{}, wlInTemplate:{departamentos:{}, docs:{}},
};
...
function renderWL(){
  ...
  if(wlTab==='in'){
    const ops=Object.values(state.wlIn).sort((a,b)=>(a.fechaCaducidad||'').localeCompare(b.fechaCaducidad||''));
    ...
    area.innerHTML=ops.map(op=>{
      const est=wlEstadoColor(op.fechaCaducidad);
      ...
      return `<div onclick="openWLSheet('${op.id}','in')" style="background:${est.bg};border:1.5px solid ${est.border};border-radius:var(--radius);padding:14px 16px;margin-bottom:10px;cursor:pointer;">
        ...
        <div style="font-size:12px;color:var(--text-muted);margin-top:4px;">Caduca: ${op.fechaCaducidad||'sin fecha'} · Departamentos: ${depsOk}/${deps.length} · Documentos: ${docsOk}/${docs.length}</div>
        ${deps.length||docs.length?`<div onclick="event.stopPropagation()" style="margin-top:10px;">
          ${deps.length?`<div style="font-size:10px;font-weight:800;color:var(--text-muted);letter-spacing:0.03em;margin-bottom:5px;">DEPARTAMENTOS</div>
          ...
          ${docs.length?`<div style="font-size:10px;font-weight:800;color:var(--text-muted);letter-spacing:0.03em;margin-bottom:5px;">DOCUMENTACIÓN AESA</div>
...
function saveWLOperador(tipo){
  ...
  const data = tipo==='in'
    ? {id, name, fechaCaducidad:document.getElementById('wl-in-fecha').value||'', departamentos, docs}
    : {id, name, departamentos, docs};
