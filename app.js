...
function moveTarea(id, dir) {
  ...
  // Asegura que todas las tareas de la columna tengan un order explícito (se mutan en el propio state.tareas)
  col.forEach((item,i) => {
    if (item.order === undefined) {
      item.order = i*10;
      DB.update(`refugio2/tareas/${item.id}`, {order: i*10}).catch(e=>console.error('No se pudo fijar el orden inicial', e));
    }
  });
  ...
  // Optimista: se mueve YA en tu pantalla, sin esperar a que Firebase responda
