...
function toggleGoalFields() {
  ...
  const vecesRow = document.getElementById('veces-row');
  if (goalRow)  goalRow.style.display  = (type === 'counter' || type === 'value') ? 'block' : 'none';
  if (vecesRow) vecesRow.style.display = (freq === 'semanal' || freq === 'mensual') ? 'block' : 'none';
}
...
function saveHabitForm() {
  ...
  const frecuenciaVeces = parseInt(document.getElementById('habit-form-veces').value) || null;
  const habit = {
    id, name,
