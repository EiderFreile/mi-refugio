...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('sw.js').then(reg => {
      // Esperar a que el SW esté activo antes de programar notificaciones
      if (reg.active) initNotifications();
      reg.addEventListener('updatefound', () => {
        reg.installing?.addEventListener('statechange', e => {
          if (e.target.state === 'activated') initNotifications();
        });
      });
    }).catch(() => {});
  }
...
// ── Notificaciones ──
const NOTIF_TIMES = [
  { hour: 9,  min: 0,  msg: '¡Buenos días! Revisa tus listas de hoy 🌿' },
  { hour: 21, min: 0,  msg: '¿Has revisado tus listas de hoy? 🌙' },
];
...
function scheduleNotifications() {
  ...
  NOTIF_TIMES.forEach(({ hour, min, msg }) => {
