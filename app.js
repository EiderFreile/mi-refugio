...
function renderTareaCard(t, opts={}) {
  ...
  const orderBtns = opts.idx !== undefined ? `<div
    ...
    style="display:flex;flex-direction:column;gap:2px;flex-shrink:0;cursor:grab;padding:0 0 0 8px;margin-left:auto;border-left:1px solid var(--border);">
    ${opts.idx > 0 ? `<button style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--text-muted);padding:1px 5px;line-height:1;" onclick="event.stopPropagation();moveTarea('${t.id}','up')">↑</button>` : '<span style="display:block;width:20px;height:16px;"></span>'}
    ${opts.idx < (opts.colLen||0)-1 ? `<button style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--text-muted);padding:1px 5px;line-height:1;" onclick="event.stopPropagation();moveTarea('${t.id}','down')">↓</button>` : '<span style="display:block;width:20px;height:16px;"></span>'}
  </div>` : '';
