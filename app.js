...
let state={
  ...
  tareas:{}, eventos:{}, recordar:{}, tareaCats:{}, porhacer:{},
};
...
function getEstadoEfectivo(t) {
  // status es la única fuente de verdad
  return t.status || 'en_espera';
}

// ── Sincroniza estado y hoy de una tarea con sus pasos ──
function syncTareaStatus(tareaId) {
  const t = state.tareas[tareaId]; if (!t) return;
  if (!t.pasos || Object.keys(t.pasos).length === 0) return;
  const estadoEfectivo = getEstadoEfectivo(t);
  const newHoy = estadoEfectivo === 'en_curso';
  if (t.status !== estadoEfectivo || t.hoy !== newHoy) {
    DB.update(`refugio2/tareas/${tareaId}`, {
      status: estadoEfectivo,
      hoy: newHoy,
      completedAt: estadoEfectivo === 'terminada' ? todayKey() : null
    });
  }
}
...
let dragSrcId = null;
let dragSrcType = null; // 'tarea' or 'paso'
let dragSrcTareaId = null;

// Get sorted tareas for a column respecting order field
...
function moveTarea(id, dir) {
  ...
  const col = Object.values(state.tareas)
    .filter(x => getEstadoEfectivo(x) === estado)
    .sort((a,b)=>(a.order!==undefined?a.order:999)-(b.order!==undefined?b.order:999));
  // Assign sequential orders first if needed
  col.forEach((item,i) => { if(item.order===undefined) DB.update(`refugio2/tareas/${item.id}`,{order:i*10}); });
  const idx = col.findIndex(x=>x.id===id);
  const swapIdx = dir==='up' ? idx-1 : idx+1;
  if(swapIdx<0||swapIdx>=col.length) return;
  const other = col[swapIdx];
  const aOrder = col[idx].order!==undefined ? col[idx].order : idx*10;
  const bOrder = col[swapIdx].order!==undefined ? col[swapIdx].order : swapIdx*10;
  DB.update(`refugio2/tareas/${id}`, {order: bOrder});
  DB.update(`refugio2/tareas/${other.id}`, {order: aOrder});
}

// Move paso up or down
function movePaso(tareaId, pasoId, dir) {
  const t = state.tareas[tareaId]; if(!t||!t.pasos) return;
  const pasos = Object.values(t.pasos).sort((a,b)=>(a.order||0)-(b.order||0));
  const idx = pasos.findIndex(p=>p.id===pasoId);
  const swapIdx = dir==='up' ? idx-1 : idx+1;
  if(swapIdx<0||swapIdx>=pasos.length) return;
  const other = pasos[swapIdx];
  const aOrder = pasos[idx].order||idx;
  const bOrder = other.order||swapIdx;
  DB.update(`refugio2/tareas/${tareaId}/pasos/${pasoId}`, {order: bOrder});
  DB.update(`refugio2/tareas/${tareaId}/pasos/${other.id}`, {order: aOrder});
}

// Drag handlers for kanban cards
...
function onDragOver(e, id) {
  ...
  if(id !== dragSrcId) document.getElementById('kcard-'+id)?.classList.add('drag-over');
}
...
function onDrop(e, targetId) {
  ...
  const src = state.tareas[dragSrcId];
  const target = state.tareas[targetId];
  if(!src||!target) return;
  const srcOrder = src.order || 0;
  const tgtOrder = target.order || 0;
  DB.update(`refugio2/tareas/${dragSrcId}`, {order: tgtOrder});
  DB.update(`refugio2/tareas/${targetId}`, {order: srcOrder});
  dragSrcId = null;
}
...
function openQuickAddWork(){
  if(state.tab==='tareas'){openTareaSheet(null);return;}
  if(state.tab==='agenda'){openTareaSheet(null);return;}
  showQuickSheet([
    {ico:'✅',lbl:'Tarea',fn:'openTareaSheet(null)'},
    {ico:'📅',lbl:'Evento en agenda',fn:'openEventoSheet(null)'},
  ]);
}
...
function toggleTareaHoy(id, currentHoy) {
  const t = state.tareas[id]; if(!t) return;
  const newStatus = !currentHoy ? 'en_curso' : 'en_espera';
  setTareaStatus(id, newStatus);
  showToast(!currentHoy ? 'En curso ☀️' : 'En espera');
}

function openSelectorHoy() {
  const pendientes = Object.values(state.tareas)
    .filter(t => t.status !== 'terminada' && !t.hoy)
    .sort((a,b) => {const p={alta:0,media:1,baja:2}; return p[a.priority]-p[b.priority];});

  if (!pendientes.length) { showToast('No hay tareas pendientes'); return; }

  const priIcon={alta:'🔴',media:'🟡',baja:'🟢'};
  const sheet = document.createElement('div');
  sheet.className = 'overlay open';
  sheet.id = 'selector-hoy-overlay';
  sheet.innerHTML = `
    <div class="sheet" onclick="event.stopPropagation()" style="max-height:80dvh;">
      <div class="sheet-handle"></div>
      <div class="sheet-title">Añadir a Hoy</div>
      <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px;">Toca las tareas que quieres trabajar hoy</p>
      ${pendientes.map(t => `
        <div onclick="toggleTareaHoy('${t.id}',false);document.getElementById('selector-hoy-overlay').remove();"
          style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--border);cursor:pointer;-webkit-tap-highlight-color:transparent;">
          <span style="font-size:16px;">${priIcon[t.priority]||''}</span>
          <div style="flex:1;">
            <div style="font-size:14px;font-weight:500;">${t.name}</div>
            ${t.date?`<div style="font-size:12px;color:var(--text-muted);">📅 ${t.date}</div>`:''}
          </div>
          <span style="font-size:20px;color:var(--lav);">☀️</span>
        </div>`).join('')}
      <button onclick="document.getElementById('selector-hoy-overlay').remove()" class="btn-ghost" style="margin-top:16px;">Cerrar</button>
    </div>`;
...
function renderHoy(){
  ...
  const hoyPendientes = hoyTareas.filter(t=>getEstadoEfectivo(t)!=='terminada');
  ...
  // En espera: tareas cuyo estado efectivo es en_espera
  const enEsperaItems = [];
  allTareas.forEach(t=>{
    const estado = getEstadoEfectivo(t);
    if(estado === 'en_espera'){
      enEsperaItems.push({tipo:'tarea', tarea:t});
    }
  });
  enEsperaItems.sort((a,b)=>{const p={alta:0,media:1,baja:2};return p[a.tarea.priority]-p[b.tarea.priority];});

  area.innerHTML=`
    ${hoyPendientes.length?`
    <div class="hoy-section">
      <div class="hoy-section-title">⭐ Hoy</div>
      ${hoyPendientes.map(t=>{
        ...
        return `<div class="hoy-task" style="flex-direction:column;align-items:stretch;" onclick="${hasPasos?'':'openTareaSheet('+JSON.stringify(t).replace(/"/g,'&quot;')+')'}">
          <div style="display:flex;align-items:center;gap:10px;">
            ${!hasPasos?`<div class="check-box" onclick="event.stopPropagation();completarTarea('${t.id}')">${checkSvg}</div>`:'<span style="font-size:16px;flex-shrink:0;">🔄</span>'}
            <span style="flex:1;font-size:14px;font-weight:600;cursor:pointer;" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">${t.name}</span>
    ...
    <div class="hoy-section">
      ...
      ${overdue.map(t=>`
        <div class="hoy-task overdue" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
          ...
          <div style="display:flex;gap:3px;flex-shrink:0;" onclick="event.stopPropagation()">
            <button onclick="setTareaStatus('${t.id}','en_curso')"
              style="padding:3px 7px;border-radius:6px;border:1px solid var(--border);background:var(--lav-light);font-size:12px;cursor:pointer;" title="En curso">🔄</button>
            <button onclick="setTareaStatus('${t.id}','terminada')"
              style="padding:3px 7px;border-radius:6px;border:1px solid var(--border);background:transparent;font-size:12px;cursor:pointer;" title="Terminada">✅</button>
          </div>
        </div>`).join('')}
    </div>`:''}
    ...
    <div class="hoy-section">
      ...
      ${enEsperaItems.map(item=>{
        const t = item.tarea;
        const pasos = t.pasos ? Object.values(t.pasos) : [];
        const nextPaso = pasos.find(p=>!p.done && p.estado!=='done');
        return `<div class="hoy-task waiting" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;gap:10px;">
            <span style="font-size:16px;flex-shrink:0;">⏳</span>
            <div style="flex:1;min-width:0;cursor:pointer;" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
              <div style="font-size:14px;font-weight:600;">${t.name}</div>
              ${nextPaso?`<div style="font-size:12px;color:var(--lav);margin-top:2px;font-weight:500;">→ ${nextPaso.text}</div>`:''}
              ${!nextPaso&&t.notes?`<div style="font-size:12px;color:var(--text-muted);margin-top:2px;">${t.notes.slice(0,50)}</div>`:''}
            </div>
            <div style="display:flex;gap:3px;flex-shrink:0;" onclick="event.stopPropagation()">
              <button onclick="setTareaStatus('${t.id}','en_curso')"
                style="padding:3px 7px;border-radius:6px;border:1px solid var(--border);background:var(--lav-light);font-size:12px;cursor:pointer;" title="Pasar a En curso">🔄</button>
              <button onclick="setTareaStatus('${t.id}','terminada')"
                style="padding:3px 7px;border-radius:6px;border:1px solid var(--border);background:transparent;font-size:12px;cursor:pointer;" title="Terminada">✅</button>
            </div>
          </div>
          ${nextPaso?`
          <div style="margin-top:8px;padding-left:26px;">
            <div style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:6px 0;" onclick="togglePasoCheck('${t.id}','${nextPaso.id}',${!!nextPaso.done})">
              <div class="check-box" style="width:16px;height:16px;border-radius:4px;flex-shrink:0;">
                <svg width="9" height="9" viewBox="0 0 11 11" fill="none"><path d="M1.5 5.5L4 8L9.5 2.5" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
              </div>
              <span style="font-size:13px;color:var(--text);">${nextPaso.text}</span>
              ${nextPaso.date?`<span style="font-size:11px;color:var(--text-muted);margin-left:auto;">📅 ${nextPaso.date}</span>`:''}
            </div>
    ...
    <div class="hoy-section" style="margin-top:16px;">
      <div class="hoy-progress">
        <div style="font-size:14px;font-weight:600;">✓ ${hoyTerminadas.length} de ${hoyTareas.length} tareas de hoy</div>
    ...
    <button onclick="openSelectorHoy()" style="width:100%;margin-top:12px;padding:12px;border-radius:10px;border:1px dashed var(--lav-mid);background:var(--lav-light);color:var(--lav);font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:600;cursor:pointer;">☀️ Añadir tarea a hoy</button>`;
...
function completarPasoYSiguiente(tareaId, pasoId){
  togglePasoCheck(tareaId, pasoId, false);
}
...
function renderTareas(){
  ...
  const taskCard = (t, idx, colLen) => {
    const isOverdue=t.date&&t.date<today&&t.status!=='terminada';
    const pasos=t.pasos?Object.values(t.pasos).sort((a,b)=>(a.order||0)-(b.order||0)):[];
    const pasosDone=pasos.filter(p=>p.done).length;
    return `<div id="kcard-${t.id}" class="kanban-card ${isOverdue?'overdue':priColors[t.priority]}"
      draggable="true"
      ondragstart="onDragStart(event,'${t.id}')"
      ondragover="onDragOver(event,'${t.id}')"
      ondragend="onDragEnd(event)"
      ondrop="onDrop(event,'${t.id}')"
      onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:6px;">
        <div class="kanban-card-name" style="${t.status==='terminada'?'text-decoration:line-through;color:var(--text-muted)':''}">${priIcon[t.priority]||''} ${t.name}</div>
        ${t.status!=='terminada'?`<button onclick="event.stopPropagation();toggleTareaHoy('${t.id}',${!!t.hoy})"
          style="background:${t.hoy?'var(--lav)':'transparent'};border:1px solid ${t.hoy?'var(--lav)':'var(--border)'};border-radius:99px;padding:2px 7px;font-size:11px;cursor:pointer;flex-shrink:0;">☀️</button>`:''}
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px;">
        <div class="kanban-card-meta">
          ${t.date?`<span>${isOverdue?'⚠️':'📅'} ${t.date}</span>`:''}
          ${t.cat?`<span>${getCatName(t.cat)}</span>`:''}
          ${pasos.length?`<span>📋 ${pasosDone}/${pasos.length}</span>`:''}
        </div>
        <div style="display:flex;gap:3px;align-items:center;" onclick="event.stopPropagation()">
          <button onclick="setTareaStatus('${t.id}','en_espera')"
            style="padding:2px 5px;border-radius:5px;border:1px solid ${t.status==='en_espera'?'var(--lav)':'var(--border)'};background:${t.status==='en_espera'?'var(--lav-light)':'transparent'};font-size:10px;cursor:pointer;">⏳</button>
          <button onclick="setTareaStatus('${t.id}','en_curso')"
            style="padding:2px 5px;border-radius:5px;border:1px solid ${t.status==='en_curso'?'var(--lav)':'var(--border)'};background:${t.status==='en_curso'?'var(--lav-light)':'transparent'};font-size:10px;cursor:pointer;">🔄</button>
          <button onclick="setTareaStatus('${t.id}','terminada')"
            style="padding:2px 5px;border-radius:5px;border:1px solid ${t.status==='terminada'?'var(--green)':'var(--border)'};background:${t.status==='terminada'?'var(--green-light)':'transparent'};font-size:10px;cursor:pointer;">✅</button>
          <div class="order-btns" onclick="event.stopPropagation()">
            ${idx>0?`<button class="order-btn" onclick="moveTarea('${t.id}','up')">↑</button>`:''}
            ${idx<colLen-1?`<button class="order-btn" onclick="moveTarea('${t.id}','down')">↓</button>`:''}
          </div>
        </div>
      </div>
      ${pasos.length?`<div style="margin-top:6px;border-top:1px solid var(--border);padding-top:6px;">
        ${pasos.map(p=>`<div style="display:flex;align-items:center;gap:6px;padding:3px 0;" onclick="event.stopPropagation();togglePasoCheck('${t.id}','${p.id}',${!!p.done})">
  ...
  const cols = [
    { status:'en_espera', label:'⏳ En espera', items: all.filter(t=>t.status==='en_espera'||t.status==='pendiente').sort((a,b)=>{const p={alta:0,media:1,baja:2};return p[a.priority]-p[b.priority];}) },
    { status:'en_curso',  label:'🔄 En curso',  items: all.filter(t=>t.status==='en_curso').sort((a,b)=>{const p={alta:0,media:1,baja:2};return p[a.priority]-p[b.priority];}) },
    { status:'terminada', label:'✅ Terminadas', items: all.filter(t=>t.status==='terminada').sort((a,b)=>(b.completedAt||'').localeCompare(a.completedAt||'')).slice(0,20) },
  ];

  board.innerHTML = `<div class="kanban-board">${cols.map(col=>`
    <div class="kanban-col">
      <div class="kanban-col-title">${col.label} <span style="font-size:11px;background:var(--border);padding:2px 6px;border-radius:99px;">${col.items.length}</span></div>
      ${col.items.length ? getSortedTareas(col.items).map((t,i)=>taskCard(t,i,col.items.length)).join('') : `<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:20px 0;">Sin tareas</div>`}
    </div>`).join('')}
  </div>`;
}
...
function togglePasos(){
  ...
  document.getElementById('pasos-container').style.display = tipo==='pasos' ? 'block' : 'none';
}

function addPaso(){
  editingPasos.push({id:Date.now().toString(), text:'', date:'', done:false});
  renderPasosList();
  const inputs = document.querySelectorAll('#pasos-list .paso-text');
  inputs[inputs.length-1]?.focus();
}

function renderPasosList(){
  document.getElementById('pasos-list').innerHTML = editingPasos.map((p,i)=>`
    <div class="paso-row" draggable="true"
      ondragstart="dragPasoStart(event,${i})"
      ondragover="dragPasoOver(event,${i})"
      ondrop="dragPasoDrop(event,${i})">
      <div class="order-btns">
        ${i>0?`<button class="order-btn" onclick="movePasoEdit(${i},-1)">↑</button>`:'<span style="width:14px;"></span>'}
        ${i<editingPasos.length-1?`<button class="order-btn" onclick="movePasoEdit(${i},1)">↓</button>`:'<span style="width:14px;"></span>'}
      </div>
      <div style="flex:1;">
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
          <input class="input-field paso-text" value="${p.text||''}" placeholder="Describe este paso..."
            style="flex:1;" oninput="editingPasos[${i}].text=this.value">
          <button class="remove-btn" onclick="editingPasos.splice(${i},1);renderPasosList()">×</button>
        </div>
        <div style="display:flex;gap:8px;">
          <input class="input-field" type="date" value="${p.date||''}"
            style="flex:1;" oninput="editingPasos[${i}].date=this.value">
          <select class="input-field" style="flex:1;" onchange="editingPasos[${i}].estado=this.value">
            <option value="en_espera" ${(!p.estado||p.estado==='en_espera')&&p.estado!=='en_curso'&&p.estado!=='terminada'&&!p.done?'selected':''}>⏳ En espera</option>
            <option value="en_curso" ${p.estado==='en_curso'&&!p.done?'selected':''}>🔄 En curso</option>
            <option value="terminada" ${p.done||p.estado==='terminada'?'selected':''}>✅ Terminada</option>
          </select>
        </div>
      </div>
    </div>`).join('');
...
let dragPasoIdx = null;
function dragPasoStart(e, i) { dragPasoIdx = i; e.dataTransfer.effectAllowed = 'move'; }
function dragPasoOver(e, i) { e.preventDefault(); }
function dragPasoDrop(e, i) {
  e.preventDefault();
  if(dragPasoIdx===null||dragPasoIdx===i) return;
  const moved = editingPasos.splice(dragPasoIdx, 1)[0];
  editingPasos.splice(i, 0, moved);
  dragPasoIdx = null;
  renderPasosList();
}
function movePasoEdit(i, dir) {
  const newI = i + dir;
  if(newI<0||newI>=editingPasos.length) return;
  const tmp = editingPasos[i];
  editingPasos[i] = editingPasos[newI];
  editingPasos[newI] = tmp;
  renderPasosList();
}
...
function renderCalMes(title,area){
  ...
  if(calSelected){
    ...
    html+=`<div class="section-label">${selDate.replace(/^\w/,c=>c.toUpperCase())}</div>`;
    if(selItems.length){
      ...
      html+=`<div style="text-align:center;padding:16px 0;color:var(--text-muted);font-size:13px;">Sin tareas este día ·
        <button onclick="openTareaSheet(null)" style="color:var(--lav);background:none;border:none;cursor:pointer;font-weight:600;">+ Añadir</button>
      </div>`;
    }
  }
  area.innerHTML=html;
...
function getCatName(catId){
  ...
  // fallback por si es un valor antiguo
  return catId;
}
