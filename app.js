  ...
  area.innerHTML=`
    ...
    <div class="hoy-section">
      ...
      ${enEsperaItems.map(item=>`
        <div class="hoy-task waiting" onclick="openTareaSheet(${JSON.stringify(item.tarea).replace(/"/g,'&quot;')})">
          <span style="font-size:16px;flex-shrink:0;">⏳</span>
          <div style="flex:1;min-width:0;">
            <div style="font-size:14px;font-weight:500;">${item.texto}</div>
            ${item.nota?`<div style="font-size:12px;color:var(--text-muted);margin-top:2px;">${item.nota.slice(0,50)}${item.nota.length>50?'...':''}</div>`:''}
          </div>
...
function renderTareas(){
  ...
  const cols = [
    { status:'en_espera', label:'⏳ En espera', items: all.filter(t=>t.status==='en_espera').sort((a,b)=>{const p={alta:0,media:1,baja:2};return p[a.priority]-p[b.priority];}) },
    { status:'en_curso',  label:'🔄 En curso',  items: all.filter(t=>t.status==='en_curso'||t.status==='pendiente').filter(t=>t.status!=='terminada'&&t.status!=='en_espera').sort((a,b)=>{const p={alta:0,media:1,baja:2};return p[a.priority]-p[b.priority];}) },
    ...
    <div style="background:var(--bg);border-radius:8px;padding:10px;margin-bottom:8px;border:1px solid var(--border);">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <select class="input-field" style="flex:1;" onchange="editingPasos[${i}].estado=this.value">
          ...
          <option value="en_espera" ${p.estado==='en_espera'?'selected':''}>⏳ En espera</option>
...
function saveTarea(){
  ...
  DB.set(`refugio2/tareas/${id}`,{
    ...
    hoy:document.getElementById('tarea-hoy').checked,
    pasos,
