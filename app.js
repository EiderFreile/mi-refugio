...
function moveTarea(id, dir) {
  ...
  if (!t) { showToast('⚠️ No encuentro esa tarea'); return; }
  const estado = getEstadoEfectivo(t);
  const col = getSortedTareas(getTareaColItems(estado));
  // Asegura que todas las tareas de la columna tengan un order explícito
  col.forEach((item,i) => {
    if (item.order === undefined) {
      item.order = i*10;
      DB.update(`refugio2/tareas/${item.id}`, {order: i*10}).catch(e=>console.error('orden inicial', e));
    }
  });
  const idx = col.findIndex(x=>x.id===id);
  if (idx === -1) { showToast('⚠️ No la encuentro en su columna'); return; }
  const swapIdx = dir==='up' ? idx-1 : idx+1;
  if (swapIdx<0 || swapIdx>=col.length) { showToast('Ya está en el extremo'); return; }
  ...
  other.order = aOrder;
  showToast(`Moviendo ${dir==='up'?'▲':'▼'} "${(t.name||'').slice(0,22)}"`);
  try {
    renderTareas();
    renderHoy();
  } catch(e) {
  ...
  DB.update(`refugio2/tareas/${id}`, {order: bOrder}).catch(e=>{showToast('❌ Fallo al guardar: '+e.message); console.error(e);});
  DB.update(`refugio2/tareas/${other.id}`, {order: aOrder}).catch(e=>{showToast('❌ Fallo al guardar: '+e.message); console.error(e);});
}
