...
function renderSummary() {
  ...
  if (elI) {
    const evitados = state.todayProgress?.impulsos
      ? Object.values(state.todayProgress.impulsos).filter(i => !i.mordio).length : 0;
...
const HABITO_ESPECIAL_ID = 'no-morderme'; // el id que tenga en Firebase

let impulsoData = { donde: null, que: null, como: null };

// Detectar si un hábito es el especial
...
// Override del render de hábitos de hoy para el especial
function renderTodayHabits() {
  const list = document.getElementById('today-habits');
  if (!list) return;
  ...
  list.innerHTML = state.habits.map(h => {
    ...
    const impulsos = state.todayProgress?.impulsos
      ? Object.values(state.todayProgress.impulsos).length : 0;
    const evitados = state.todayProgress?.impulsos
      ? Object.values(state.todayProgress.impulsos).filter(i => !i.mordio).length : 0;

    return `
      <div class="habit-item ${done && !especial ? 'completed' : ''}" id="habit-item-${h.id}">
        <div class="habit-icon" style="background:${h.color}20;">${h.icon}</div>
        <div class="habit-info">
          <div class="habit-name">${h.name}</div>
          ${especial ? `
            <div class="habit-progress-text">
              ${impulsos ? `${evitados}/${impulsos} impulsos evitados hoy` : 'Sin impulsos registrados hoy'}
            </div>
          ` : h.type === 'counter' ? `
            <div class="habit-progress-text">${current} / ${h.goal} ${h.unit || ''}</div>
            <div class="habit-bar-wrap">
...
function renderOpts(containerId, options, onSelect) {
  ...
  container.innerHTML = options.map(opt => `
    <button onclick="(${onSelect.toString()})('${opt}')"
      ...
      onmousedown="this.style.background='var(--c-sage-dim)';this.style.borderColor='var(--c-sage)'"
      onmouseup="this.style.background='var(--c-surface)';this.style.borderColor='var(--c-border)'">
      ${opt}
    </button>
  `).join('');
}
...
function guardarImpulso(seMordio) {
  ...
  if (seMordio) {
    ...
    // Abrir SOS si quiere relajarse igualmente
    setTimeout(() => openSOS(), 500);
  }
...
let diarioEntradas = {}; // cargadas desde Firebase
let diarioMoodSeleccionado = null;
let diarioTagsSeleccionados = [];

// Listener Firebase
function initDiarioListener() {
...
function renderDiario() {
  ...
  const entradas = Object.entries(diarioEntradas)
  ...
  list.innerHTML = entradas.map(([id, e]) => {
    const fecha = new Date(e.timestamp).toLocaleDateString('es-ES', {
      ...
      <div class="card" style="margin-bottom:12px;cursor:pointer;" onclick="abrirEntrada('${id}')">
        ...
        ${e.tags?.length ? `
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px;">
            ${e.tags.map(t => `
              <span style="font-size:11px;background:var(--c-sage-dim);color:var(--c-sage);
                           padding:3px 10px;border-radius:99px;font-weight:500;">${t}</span>
            `).join('')}
          </div>
        ` : ''}
      </div>`;
...
function filtrarEntradas(query) {
  ...
  diarioEntradas = Object.fromEntries(filtradas);
  renderDiario();
  diarioEntradas = {}; // reset — se recargará del listener
  DB.listen('refugio/diario', data => { diarioEntradas = data || {}; });
}

// Abrir sheet nueva entrada
...
function renderDiarioTagsGrid() {
  ...
  grid.innerHTML = DIARIO_TAGS.map(t => {
    ...
    return `
      <button onclick="toggleDiarioTag('${t}')"
        style="padding:6px 14px;border-radius:99px;font-size:12px;font-weight:500;cursor:pointer;
               ...
               color:${sel ? 'var(--c-sage)' : 'var(--c-text-muted)'};
               transition:all 0.15s;">${t}</button>`;
  }).join('');
}

function toggleDiarioTag(tag) {
  if (diarioTagsSeleccionados.includes(tag)) {
    diarioTagsSeleccionados = diarioTagsSeleccionados.filter(t => t !== tag);
  } else {
    diarioTagsSeleccionados.push(tag);
  }
  renderDiarioTagsGrid();
}
...
function guardarEntradaDiario() {
  ...
  const entrada = {
    texto,
    mood: diarioMoodSeleccionado,
    tags: diarioTagsSeleccionados,
    timestamp: diarioEntradas[id]?.timestamp || Date.now(),
    updatedAt: Date.now(),
    fecha: todayKey()
  };

  DB.set(`refugio/diario/${id}`, entrada).then(() => {
    closeDiarioSheet();
...
function eliminarEntradaDiario() {
  ...
  DB.remove(`refugio/diario/${id}`).then(() => {
    closeDiarioSheet();
