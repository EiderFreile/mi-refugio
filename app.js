...
function renderTareaCard(t, opts={}) {
  ...
  const orderBtns = opts.idx !== undefined ? `<div
    draggable="true"
    ondragstart="event.stopPropagation();onDragStart(event,'${t.id}')"
    onclick="event.stopPropagation()"
    style="display:flex;flex-direction:column;gap:2px;flex-shrink:0;cursor:grab;padding:0 2px;">
    ${opts.idx > 0 ? `<button style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--text-muted);padding:1px 3px;line-height:1;" onclick="event.stopPropagation();moveTarea('${t.id}','up')">↑</button>` : '<span style="display:block;width:20px;height:16px;"></span>'}
    ${opts.idx < (opts.colLen||0)-1 ? `<button style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--text-muted);padding:1px 3px;line-height:1;" onclick="event.stopPropagation();moveTarea('${t.id}','down')">↓</button>` : '<span style="display:block;width:20px;height:16px;"></span>'}
  </div>` : '';
  ...
  c.innerHTML=editingPasos.map((p,i)=>`
    <div class="item-row">
      <div style="display:flex;flex-direction:column;gap:2px;flex-shrink:0;">
        ${i>0 ? `<button style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--text-muted);padding:1px 3px;line-height:1;" onclick="movePasoEdit(${i},'up')">↑</button>` : `<span style="display:block;width:18px;height:14px;"></span>`}
        ${i<editingPasos.length-1 ? `<button style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--text-muted);padding:1px 3px;line-height:1;" onclick="movePasoEdit(${i},'down')">↓</button>` : `<span style="display:block;width:18px;height:14px;"></span>`}
      </div>
