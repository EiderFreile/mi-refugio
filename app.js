...
function togglePasoCheck(tareaId, pasoId, currentDone){
  const t = state.tareas[tareaId]; if(!t) return;
  DB.update(`refugio2/tareas/${tareaId}/pasos/${pasoId}`, {done: !currentDone}).then(()=>{
    if(!currentDone){
      const pasos = t.pasos ? Object.values(t.pasos) : [];
      const restantes = pasos.filter(p=>p.id!==pasoId&&!p.done);
      if(restantes.length===0){
        DB.update(`refugio2/tareas/${tareaId}`,{status:'terminada',completedAt:todayKey()});
        showToast('¡Todos los pasos completados! 🎉',3000);
      } else {
        showToast(`Paso completado ✓ · Quedan ${restantes.length}`);
      }
    }
  });
}
...
function renderHoy(){
  ...
  area.innerHTML=`
    ...
    <div class="hoy-section">
      ...
      ${hoyPendientes.map(t=>`
        <div class="hoy-task" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
      ...
      ${enEsperaItems.map(item=>{
        ...
        return `<div class="hoy-task waiting" style="flex-direction:column;align-items:stretch;">
          ...
          <div style="margin-top:8px;padding-left:26px;">
            <div style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:6px 0;" onclick="completarPasoYSiguiente('${t.id}','${nextPaso.id}')">
...
function completarPasoYSiguiente(tareaId, pasoId){
  const t = state.tareas[tareaId]; if(!t) return;
  DB.update(`refugio2/tareas/${tareaId}/pasos/${pasoId}`, {done: true, estado: 'terminada'}).then(()=>{
    // Firebase listener will update state, then we sync status
    setTimeout(()=>{
      const tUpdated = state.tareas[tareaId];
      if(!tUpdated) return;
      const pasos = tUpdated.pasos ? Object.values(tUpdated.pasos) : [];
      const restantes = pasos.filter(p=>p.id!==pasoId && !p.done && p.estado!=='terminada');
      if(restantes.length===0){
        DB.update(`refugio2/tareas/${tareaId}`,{status:'terminada',completedAt:todayKey(),hoy:false});
        showToast('¡Todos los pasos completados! 🎉',3000);
      } else {
        // Update task status to reflect next paso
        const nextEstado = restantes[0].estado || 'en_espera';
        const newHoy = nextEstado === 'en_curso';
        DB.update(`refugio2/tareas/${tareaId}`,{status:nextEstado, hoy:newHoy});
        showToast(`✓ · Siguiente: ${restantes[0].text}`);
      }
    }, 500);
  });
}
...
function renderTareas(){
  ...
  const taskCard = t => {
    ...
    return `<div class="kanban-card ${isOverdue?'overdue':priColors[t.priority]}" onclick="openTareaSheet(${JSON.stringify(t).replace(/"/g,'&quot;')})">
  ...
  board.innerHTML = `<div class="kanban-board">${cols.map(col=>`
    <div class="kanban-col">
      ...
      ${col.items.length ? col.items.map(taskCard).join('') : `<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:20px 0;">Sin tareas</div>`}
    </div>`).join('')}
  </div>`;
}
...
function renderPasosList(){
  document.getElementById('pasos-list').innerHTML = editingPasos.map((p,i)=>`
    <div style="background:var(--bg);border-radius:8px;padding:10px;margin-bottom:8px;border:1px solid var(--border);">
...
function openTareaSheet(t){
  ...
  if(statusEl) {
    statusEl.value = hasPasosEdit ? getEstadoEfectivo(t) : (t?.status||'en_espera');
    statusEl.disabled = hasPasosEdit;
    statusEl.style.opacity = hasPasosEdit ? '0.5' : '1';
    statusEl.title = hasPasosEdit ? 'El estado se hereda del paso activo' : '';
  }
...
function saveTarea(){
  ...
  const pasos = hasPasos
    ? Object.fromEntries(pasosArr.map(p=>[p.id,{
