...
function shouldShowToday(cl) {
  ...
  if (!isNaN(days)) {
    const created = cl.createdAt ? new Date(cl.createdAt) : new Date();
...
function renderChecklistsHome() {
  ...
  container.innerHTML = today.map(cl => {
    ...
    const prog = state.progress[cl.id] || {};
    const done = items.filter(it => prog[it.id]).length;
    const color = COLORS.find(c => c.id === cl.color)?.hex || COLORS[0].hex;

    return `
      <div class="checklist-card" id="clcard-${cl.id}">
        <div class="checklist-header" onclick="toggleChecklist('${cl.id}')">
          <div class="checklist-color" style="background:${COLOR_DOTS[COLORS.findIndex(c=>c.id===cl.color)||0]||'#9B8EC4'};"></div>
          <div class="checklist-info">
            <div class="checklist-name">${cl.name}</div>
            <div class="checklist-meta">${freqLabel(cl.frecuencia)} · ${done}/${items.length} completados</div>
          </div>
          <div class="checklist-progress">${progressRing(done, items.length, color)}</div>
        </div>
        <div class="checklist-items" id="items-${cl.id}">
          ${items.map(it => {
            const checked = !!prog[it.id];
            return `<div class="check-item" onclick="toggleItem('${cl.id}','${it.id}')">
...
function toggleItem(clId, itemId) {
  const current = !!(state.progress[clId]?.[itemId]);
  DB.update(`refugio2/progress/${todayKey()}/${clId}`, { [itemId]: !current });
}
