...
// ─── Actualizar renderStaticHome para incluir paso ──────────
const _originalRenderStaticHome = renderStaticHome;
function renderStaticHome() {
  const hour = new Date().getHours();
  const greet = hour < 13 ? 'Buenos días' : hour < 20 ? 'Buenas tardes' : 'Buenas noches';
...
// ─── Mejorar selectMood con microanimación ──────────────────
const _origSelectMood = selectMood;
function selectMood(id) {
  _origSelectMood(id);
  // El CSS ya maneja la animación via .selected
}
